...
if __name__ == '__main__':
    ...
    img = cv2.imread(
        r'D:\wltlu\Work\CarND\Term3\Lyft_Changllenge\LyftChallengeV2-master\Train\_1\CameraRGB\0.png')
    gt = cv2.imread(
        r'D:\wltlu\Work\CarND\Term3\Lyft_Changllenge\LyftChallengeV2-master\Train\_1\CameraSeg\0.png', 1)
    gt = np.uint8(np.all(gt == background_color, axis=2))
    g2rgb = lambda img: np.dstack((255 * img, 0 * img, 255 * img))
    put_text = lambda img, text: cv2.putText(img, text, (10, 50), cv2.FONT_HERSHEY_DUPLEX, 2.0, (255, 127, 127), 3,
                                             cv2.LINE_AA)  # noqa
